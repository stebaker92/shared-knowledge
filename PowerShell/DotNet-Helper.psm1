...
function dotnetpublish($service) {
    ...
    Replace-JsonValue "$releasesFolder\$service\appsettings.json" -key:"JwtSecret" -value:$jwtSecret
    Replace-JsonValue "$releasesFolder\$service\appsettings.json" -key:"ClientSecret" -value:$clientSecret
    ...
    if ($appsettingsdev | Test-Path) { 
        ...
        Replace-JsonValue "$releasesFolder\$service\appsettings.development.json" -key:"JwtSecret" -value:$jwtSecret
        Replace-JsonValue "$releasesFolder\$service\appsettings.development.json" -key:"ClientSecret" -value:$clientSecret
    }
...
# TODO move these out to modules
function Replace-JsonValue([string]$jsonFilePath, [string]$key, [string]$value) {
    (Get-Content $jsonFilePath ) | foreach-object {
  
        if ($_ -match '"' + $key + '"\: "(.+)"') {
            Write-Host "$_ is a match"
            Write-Host "New value is: $value"
            $_ -replace ('"' + $key + '"\: "(.+)"'), [string]::Format('"' + $key + '": "{0}"', $value) #brackets are important!
        }
        else {
            $_
        }
